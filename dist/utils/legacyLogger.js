...
export const logger = {
    info: (message, ...args) => {
        ...
        structuredLogger.info(LogCategory.SYSTEM, detectComponent(), 'log', message, args.length > 0 ? { extraArgs: args } : undefined);
    },
    error: (message, ...args) => {
        // Preserve original console output
        ...
        structuredLogger.error(LogCategory.SYSTEM, detectComponent(), 'log', message, error, args.length > 0 ? { extraArgs: args } : undefined);
    },
    warn: (message, ...args) => {
        // Preserve original console output
        console.error(`${LOG_PREFIX} [WARN] ${message}`, ...args);
        // Log to structured logger
        structuredLogger.warn(LogCategory.SYSTEM, detectComponent(), 'log', message, args.length > 0 ? { extraArgs: args } : undefined);
    },
    debug: (message, ...args) => {
        if (process.env.DEBUG) {
            ...
            // Log to structured logger
            structuredLogger.debug(LogCategory.SYSTEM, detectComponent(), 'log', message, args.length > 0 ? { extraArgs: args } : undefined);
        }
    },
    progress: (message) => {
        // Preserve original console output
        ...
        // Log to structured logger
        structuredLogger.info(LogCategory.SYSTEM, detectComponent(), 'progress', message);
    }
};
//# sourceMappingURL=legacyLogger.js.map